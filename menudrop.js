...
directiveModule.directive('ngDropdown', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
	return {
		restrict: 'AE',
		scope: {
			selectedModel: '=',
			...
			$scope.setSelectedItem = function(id, dontRemove, fireSelectionChange) {
				...
				if ($scope.singleSelection) {
					...
					if (!dontRemove && exists) {
						...
						if (fireSelectionChange == true) { console.log('123');$scope.externalEvents.onItemSelect(finalObj) };
